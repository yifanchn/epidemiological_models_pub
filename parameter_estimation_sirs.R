# parameter estimation for the SIRS model
...
model_SIRS <- function(t, x, params){
  S <- x[1]
  I <- x[2]
  R <- x[3]
  ...
  dSdt <- -beta_t * S * I / N + gamma * R
  dIdt <- beta_t * S * I / N - (1/10) * I
  dRdt <- (1/10) * I - gamma * R
...
SSE <- function(parameters){
  names(parameters) <- c("beta0", "gamma") 
  times <- seq(from=1, to=83, by=1)
  xstart <- c(S=N-I_start, I=I_start, R=0)
  out <- as.data.table(ode(func = model_SIRS,
                           y = xstart,
                           times = times,
                           parms = parameters))
  ...
  #SSE <- sum((data_validation[, casesperday] -
  #              data_validation[, DifferenzVortagFaelle])^2)
  return(SSE)
}

# optimize the parameter beta with the L-BFGS-B method
opt <- optim(c(beta0=0.1, gamma=0.1), SSE, method="L-BFGS-B", lower=c(0, 0), upper=c(3, 3))

opt_par <- setNames(opt$par, c("beta0", "gamma"))
...
ggplot() +
  ...
  geom_line(data=out, aes(x=time, y=casesperday, colour="model (SIRS)")) +
  labs(x="days between 2021/10/13 and 2022/01/04",
...
ggplot() +
  ...
  geom_line(data=out, aes(x=time, y=cumulatedcases, colour="model (SIRS)")) +
  labs(x="days between 2021/10/13 and 2022/01/04",
